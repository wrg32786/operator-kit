...
mkdir -p "$HOOKS_DEST"
cp "$REPO_DIR/context-loader/auto-context-load.sh" "$HOOKS_DEST/auto-context-load.sh"
...
if [ ! -f "$KEYWORDS_DEST" ]; then
  cp "$REPO_DIR/context-loader/project-keywords.json" "$KEYWORDS_DEST"
...
RULES_TEMPLATE="$REPO_DIR/rules/post-compact-critical.md.template"
if [ -t 0 ]; then
  # Interactive terminal — offer the template
  echo ""
  ...
  # Non-interactive (piped curl install) — skip the prompt
  info "  Non-interactive install detected. To add the rules template:"
  info "  cp $RULES_TEMPLATE ./rules/post-compact-critical.md"
  info "  Then add '@rules/post-compact-critical.md' to your CLAUDE.md"
fi
